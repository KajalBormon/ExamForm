...
<body>
    ...
    <div class="wrapper">
        ...
        <?php
            $select = "SELECT * FROM sub_reg WHERE role='Improve' AND username='{$user}'";
            $sql = mysqli_query($conn,$select);
            $i=1;
            $num_rows = mysqli_num_rows($sql);
            if(!$num_rows){
            echo "<div style='color: red; font-weight: bold; font-size: 30px;'>You have not Registered for Improvement...!!</div>";
            }else{   
        ?>
            <h1 class="myh1">Improvement Computer Science & Engineering Subjects</h1>
            <div class="forms">
