...
<body>
    ...
    <div style="display:flex;">
        <div>
            <h2 style="color:#ff0000;font-family:'Acme';" class = "page-header mr-5"><u>Students Registered For 4th Year 2nd Semester</u></h2>
        </div>
        <div class="search_form" class="input-group">
            <form action="" method="GET">
                <input type="hidden" name="page" value="see_s8"> 
    ...
    <?php
        ...
        if($value){
            $query1 = "SELECT * FROM registration JOIN sem_reg ON registration.username = sem_reg.username WHERE semester='8' and continue_batch='{$value}'";
    
            $result = mysqli_query($conn,$query1);
            $num_rows = mysqli_num_rows($result);
            if(!$num_rows){
                echo "<h2 style='color:red;color:#ff0000;font-family:Acme;position:relative;top:35px;left:2%;''>No Students Have Registered For 4th Year 2nd Semester Exam Yet.</h2>";
            }else{
    ?>
    <div class = "col-lg-12 mcon text-center">
        ...
        <?php 
            ...
            $query1 = "SELECT * FROM registration JOIN sem_reg ON registration.username = sem_reg.username WHERE semester='8' order by continue_batch";
            $result = mysqli_query($conn,$query1);
            $num_rows = mysqli_num_rows($result);
            $result = mysqli_query($conn,$query1);
            $num_rows = mysqli_num_rows($result);
